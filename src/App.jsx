...
const buildQuickAddBookmarklet = (baseUrl) => {
  ...
  return `javascript:(()=>{const c=s=>(s||'').replace(/\\s+/g,' ').trim(),q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)],u=s=>{try{return new URL(s,location.href).href}catch{return''}},fix=s=>{const v=u(s);if(!v||v.startsWith('data:'))return'';try{const x=new URL(v);if(/(^|\\.)media-amazon\\.|(^|\\.)ssl-images-amazon\\.|(^|\\.)images-amazon\\./i.test(x.hostname))x.pathname=x.pathname.replace(/\\._[^/]+_\\.(jpg|jpeg|png|webp)$/i,'.$1');return x.href}catch{return v}},m=n=>q('meta[property="'+n+'"],meta[name="'+n+'"]')?.content||'',flat=x=>Array.isArray(x)?x.flatMap(flat):x&&typeof x==='object'&&Array.isArray(x['@graph'])?flat(x['@graph']):[x],json=()=>{try{return qa('script[type="application/ld+json"]').flatMap(s=>flat(JSON.parse(s.textContent||'{}'))).find(x=>x&&/Product/i.test(Array.isArray(x['@type'])?x['@type'].join(' '):x['@type']||''))||{}}catch{return{}}},price=t=>{const x=c(t).match(/([0-9]{1,3}(?:[.,][0-9]{3})*(?:[.,][0-9]{1,2})|[0-9]+)/);if(!x)return'';const r=x[1];return r.lastIndexOf(',')>r.lastIndexOf('.')?r.replace(/\\./g,'').replace(',','.'):r.replace(/,/g,'')},first=o=>Array.isArray(o)?o[0]:o||{},bestImg=(seed)=>{const picks=[],add=(src,score=0)=>{const url=fix(src);if(url&&!picks.some(p=>p.url===url))picks.push({url,score})},main=q('#landingImage,#imgBlkFront,#ebooksImgBlkFront,#main-image,#imgTagWrapperId img,[data-a-dynamic-image]');add(main?.getAttribute('data-old-hires'),900000);try{Object.entries(JSON.parse(main?.getAttribute('data-a-dynamic-image')||'{}')).forEach(([src,dim])=>add(src,800000+((dim?.[0]||0)*(dim?.[1]||0))))}catch{}add(seed,700000);add(m('og:image'),650000);add(m('twitter:image'),640000);add(main?.currentSrc||main?.src,600000);qa('img').forEach((img,i)=>{const src=img.currentSrc||img.src||img.getAttribute('data-src')||img.getAttribute('data-old-hires'),txt=[img.id,img.className,img.alt,src].join(' '),area=(img.naturalWidth||parseInt(img.getAttribute('width')||0,10)||0)*(img.naturalHeight||parseInt(img.getAttribute('height')||0,10)||0);if(area>2500&&/(product|landing|main|hero|image|media|amazon|imgBlk|imageBlock)/i.test(txt))add(src,area-i)});picks.sort((a,b)=>b.score-a.score);return picks[0]?.url||''},o=json(),offer=first(o.offers),image=first(o.image),data={name:c(o.name||m('og:title')||m('twitter:title')||q('#productTitle,h1')?.textContent||document.title).replace(/\\s*[:|-]\\s*Amazon\\..*$/i,''),price:price(offer.price||m('product:price:amount')||m('og:price:amount')||q('#corePrice_feature_div .a-offscreen,.a-price .a-offscreen,[itemprop="price"],.price,[class*="price"]')?.textContent),url:location.href,imageUrl:bestImg(image),category:c(o.category||m('product:category')||q('#wayfinding-breadcrumbs_container,.breadcrumb,[class*="breadcrumb"]')?.textContent)},target=${laterbaseUrl}+'#quickAdd='+encodeURIComponent(JSON.stringify(data)),w=window.open(target,'_blank');if(w)w.opener=null;else location.assign(target);})();`;
};
